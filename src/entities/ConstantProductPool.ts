...
export class ConstantProductPool implements Pool {
  ...
  public getOutputAmount(inputAmount: CurrencyAmount<Token>): [CurrencyAmount<Token>, ConstantProductPool] {
    ...
    if (JSBI.equal(this.reserve0.quotient, JSBI.BigInt(0)) || JSBI.equal(this.reserve1.quotient, JSBI.BigInt(0))) {
    ...
    if (JSBI.equal(outputAmount.quotient, JSBI.BigInt(0))) {
  ...
  public getInputAmount(outputAmount: CurrencyAmount<Token>): [CurrencyAmount<Token>, ConstantProductPool] {
    ...
    if (
      JSBI.equal(this.reserve0.quotient, JSBI.BigInt(0)) ||
      JSBI.equal(this.reserve1.quotient, JSBI.BigInt(0)) ||
  ...
  public getMintFee(reserve0: JSBI, reserve1: JSBI, totalSupply: JSBI): JSBI {
    ...
    if (JSBI.notEqual(this.kLast, JSBI.BigInt(0))) {
      ...
      if (JSBI.greaterThan(computed, this.kLast)) {
        ...
        if (JSBI.notEqual(liquidity, JSBI.BigInt(0))) {
          return liquidity
        }
      }
    }
  ...
  public getLiquidityMinted(
    ...
    if (JSBI.equal(totalSupply.quotient, JSBI.BigInt(0))) {
    ...
    if (!JSBI.greaterThan(liquidity, JSBI.BigInt(0))) {
      throw new InsufficientInputAmountError()
    }
